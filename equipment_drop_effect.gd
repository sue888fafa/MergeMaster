...
var item_id := ""
var item_name := ""
var quality_color := Color("#fbbf24")
var icon: Texture2D
...
func setup(next_item_id: String, next_landing_position: Vector2) -> void:
	...
	quality_color = Color(str(item.get("quality_color", "#fbbf24")))
...
func _draw() -> void:
	...
	draw_rect(Rect2(-21.0, -21.0, 42.0, 42.0), quality_color, false, 2.0)
	if icon != null:
		draw_texture_rect(icon, Rect2(-18.0, -18.0, 36.0, 36.0), false)
	else:
		draw_circle(Vector2.ZERO, 12.0, Color("#fbbf24"))
	var font := ThemeDB.fallback_font
	draw_string(font, Vector2(-42.0, 48.0), item_name, HORIZONTAL_ALIGNMENT_CENTER, 84.0, 12, quality_color)
